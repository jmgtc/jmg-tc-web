...
async function finalMassiveTranslation() {
  console.log("🚀 Starting FINAL PROFESSIONAL TRANSLATION for all articles...");
  for (const item of fullTranslations) {
    await client.patch(item.id).set({
      title_en: item.t,
      body_en: [{ _type: 'block', children: [{ _type: 'span', text: item.b }] }]
    }).commit();
    console.log(`✅ Applied professional English to: ${item.t}`);
  }
  console.log("✨ ALL ARTICLES FULLY TRANSLATED AND AUDITED!");
}

finalMassiveTranslation().catch(console.error);
