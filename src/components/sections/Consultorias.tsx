      ...
      {servicios.map((srv, i) => (
        ...
        >
          ...
          <div className="flex-1">
            <span className="text-[10px] font-mono text-brand-gray-body/50 uppercase tracking-[0.4em] block mb-4">
              {srv.tag}
            </span>
            <h2 className="text-4xl font-bold text-brand-gray-title mb-2">{srv.titulo}</h2>
            <p className="text-brand-gray-body mb-10">{srv.subtitulo}</p>
            <div className="grid grid-cols-2 gap-4">
              {srv.fases.map((fase) => (
                <div key={fase.num} className="p-4 rounded-2xl bg-white border border-black/5 hover:border-gold transition-colors group">
                  <span className="text-gold font-bold text-xs">{fase.num}</span>
                  <h3 className="font-bold text-brand-gray-title mt-1 mb-1 group-hover:text-gold transition-colors">{fase.label}</h3>
          ...
          <div className="flex-1 flex items-center justify-center">
            <div className="w-72 h-72 rounded-3xl bg-gradient-to-br from-black to-gray-800 flex items-center justify-center shadow-2xl">
              <span className="text-gold font-bold text-8xl opacity-20">{srv.id}</span>
            </div>
          </div>
        </div>
      ))}
